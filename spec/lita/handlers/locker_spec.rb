...
describe Lita::Handlers::Locker, lita_handler: true do
  ...
  describe '#lock' do
    ...
    it 'does not add a user multiple times to the end of a queue' do
      ...
      expect(replies.last).to eq('(lock) bazbat is locked by Alice (taken less than a minute ago). Next up: Bob')
    end
    ...
    it 'shows a warning when a label is taken by someone else' do
      ...
      send_command('lock bazbat', as: alice)
      send_command('lock bazbat', as: bob)
      expect(replies.last).to eq('(failed) bazbat is locked by Alice (@alice) (taken less than a minute ago), you have been ' \
  ...
  describe '#unlock' do
    ...
    it 'does not unlock a label when someone else locked it' do
      ...
      expect(replies.last).to eq('(failed) bazbat is locked by Alice (@alice) (taken less than a minute ago)')
    end
  ...
  describe '#steal' do
    ...
    it 'preserves the state of the queue when there is one' do
      ...
      send_command('locker status bazbat')
      expect(replies.last).to eq('(lock) bazbat is locked by Charlie (taken less than a minute ago). Next up: Bob')
    end
